...
async function bootstrap() {
  ...
  try {
    ...
    try { await app.close(); } catch (closeErr) { logger.debug(`App close error after listen failure: ${sanitizeForLogOutput(closeErr instanceof Error ? closeErr.message : String(closeErr))}`); }
    process.exit(1);
  }
}
