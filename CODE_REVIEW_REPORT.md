...
1. **🟡 `product-tools.ts` — hardcoded `100` in search products schema instead of shared constant.**
    `searchProductsSchema` used `optionalSearchFilterString(100)` for the
    `productType` filter length cap while `MAX_PRODUCT_TYPE_LENGTH = 100` was
    already imported from `@besterp/shared` and used everywhere else in the
    same file. A hardcoded literal diverges from the constant if the cap is
    ever changed, leaving the schema silently out of sync with the service
    layer. Replaced the literal with `MAX_PRODUCT_TYPE_LENGTH`.

2. **🟡 `agent-tools.ts` — `.describe()` placed inside `.pipe()` instead of outside in helper functions.**
    `agentIdParam` and `agentNameParam` both called `.describe(description)` on
    the inner schema (inside `.pipe()`), while the equivalent `uuidParam` helper
    in `packages/mcp-tools/src/schema/schema-builders.ts` and every other
    direct usage in `agent-tools.ts` (e.g. `description` and `version` fields
    in `registerAgentSchema`) place `.describe()` on the outer schema. This
    inconsistency meant the description metadata ended up attached to a
    different Zod schema node, producing subtly different schema shapes in the
    agent-facing tool registry output. Moved `.describe()` to the outer
    invocation in both helpers, matching the established pattern.
...
## Findings & Actions (round 242)

### Fixed this round

1. **🟡 `security.service.ts` — `updateAgent` called `validateAgentLimits` twice when both limit fields were provided.**
    The two independent `if` branches for `maxToolCallsPerConversation` and
    `rateLimitPerMinute` both passed the full pair of values to
    `validateAgentLimits`, so when an update supplied both fields the helper
    ran twice with identical arguments. `validateAgentLimits` itself is
    idempotent (it skips each range check when the value is `undefined`), so
    correctness was unaffected, but the duplicate call was a wasted operation
    and a readability issue. Collapsed the two branches into a single
    `||`-guarded conditional that calls `validateAgentLimits` once, passing
    the current value of each field (or `undefined` when the field was not
    supplied), matching the same single-call pattern already used in
    `registerAgent`.
...
1. **🟡 `product.service.ts:63` — `createProduct` cyclomatic complexity was 16, exceeding the lint max of 15.**
    The `createProduct` method contained sequential `if` branches for validating
    `name`, `description`, `sku`, the product type lookup, and features, plus
    ternary expressions for optional fields — pushing its complexity past the
    `complexity: ["warn", { max: 15 }]` rule. Extracted all field validation
    and trimming logic into a private static `validateCreateProductInput(input)`
    helper. `createProduct` now delegates to it; complexity drops to 5.
    Behaviour is unchanged; all existing `createProduct` spec tests continue
    to pass.
...
---
    ...
    body does not reference `this` — it only calls `SecurityService.requireStringField` and
    performs local field mapping. Changed to `private static buildUpdateData` and updated
    the sole call site in `updateAgent` from `this.buildUpdateData(updates)` to
    `SecurityService.buildUpdateData(updates)`, matching the established pattern.

2. **🟡 `party.service.ts` — `toPartyResult` used `this.toPersonResult`/`this.toOrgResult` instead of class-name invocation.**
    `PartyService.toPartyResult` (line 1351) is a `private static` method that called
    `this.toPersonResult(...)` and `this.toOrgResult(...)` (also `private static`). While
    this is functionally correct in TypeScript, it diverges from the class-name invocation
    pattern used everywhere else in the codebase (`ProductService.toXxx(...)`,
    `SecurityService.toXxx(...)`). Changed both calls to `PartyService.toPersonResult(...)`
    and `PartyService.toOrgResult(...)` for consistency.
...
## Findings & Actions (round 192)

### Fixed this round

1. **🟢 `party.service.ts:541` — stale "lone exception" comment on `getParty` tenantId filter.**
   The comment still described `getParty` as the sole query missing an
   app-level `tenantId` filter, but round 32 added it alongside the other
   queries. Updated the comment to reflect current reality.

2. **🟢 Corrected shared test count (251→243) in review report.**
   Round 191's test-count line said 251; the actual count is 243 (13
   pluralize + 76 shared + 124 sanitize + 30 crypto). Fixed the reported
   number so it matches the live `npm test` output.
...
## Findings & Actions (round 193)

### Fixed this round

None — comprehensive pass over the entire codebase confirmed no new issues.
...
1. **🟢 Removed stale `.opencode/plans/code-review-round-73.md`.** The plan file
   from round 73 (116 rounds ago) was completely outdated — it referenced
   "Phase 0b" notes in `prisma.module.ts` that had been replaced in a much
   earlier round, and all its findings had already been resolved or superseded.
   Kept the file would add no value and risk confusing future reviewers.
